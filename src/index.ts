...
// - Shows exact entry minute 20/21/22/23/24/25.
// - Shows exact Hunter Score values (60,61,62,63,64,65...) and Score × minute matrix.
// - Shows ENTRY / GOAL / NO GOAL / OPEN / success / avg time to goal.
// - NO odds requirement.
// - NO live strategy, Matcher, Telegram ENTRY, BET READY or /betstats change.
//
// // V6.7.10.34 — RESTORE FULL /STATS CAPTURE 10–42′ SCORE >=60
...
// - LIVE Telegram / Matcher promotion remains ONLY 10–21′ + Score >=64.
...
//
// // V6.7.10.34 — YESTERDAY PIPELINE DIAGNOSTIC
// Adds /yesterdaycheck (Telegram) and GET /diagnostics/yesterday.
// Read-only D1 retrospective for yesterday's 10–21′ Score >=64 signals.
...
//
// // V6.7.10.34 — /STATS SCORE >=60 + ENTRY MINUTE FIX
// - /stats is explicitly Hunter history Score >=60.
// - LIVE / Telegram / BET READY remain 10–21′ + Score >=64.
// - Score 60–63 continues to be D1/result-tracking observation only.
// - Fixes monthly and daily /stats entry-minute groups:
//   10–19, 20–24, 25–29, 30–34, 35–42.
// - /today and /betstats remain unchanged.
//
// // V6.7.10.34 — SCORE 60–64 OBSERVATION
// LIVE remains 10–21′ + Score >=64.
// Score 60–63 is D1/result-tracking research only (GOAL HIT / NO GOAL).
// No Telegram, Cloudbet Matcher, Bet Worker, odds lookup or BET READY for 60–64.
// /stats splits 60–64 and 65–69 so the 5-point difference is measurable.
// /today and /betstats now use the real >=64 BET READY population.
//
// 

// V6.7.10.34 behavior:
// - Telegram: ALL Hunter signals passing 10–21′ + Score >=64
...
// ============================================================
// GOAL WATCH — HUNTER TRACKER V6.7.10.34 TELEGRAM ALL FILTERED + FULL STATS
...
// - IMPORTANT: restores the real 20–24 BET READY threshold to Score >= 64.
...
// V6.7.10.34 — /stats keeps the COMPLETE normal Hunter history.
// These groups are reporting-only and DO NOT change the live 10–21′ Score >=64 filter.
...
export default {
  ...
  async fetch(request, env) {
    ...
    // ========================================================
    // V6.7.10.34 — LIVE HUNTER FILTER DIAGNOSTICS
    //
    // READ ONLY:
    // V27 received -> 1H -> 0:0 -> 10–21' -> Score >=64
    // -> already tracking -> new eligible
    //
    // Does NOT create signals, send Telegram entries or touch Bet Worker.
    // ========================================================
    // V6.7.10.34 — YESTERDAY PIPELINE DIAGNOSTIC (D1, READ ONLY)
    // Shows what happened to yesterday's stored 10–21′ Score >=64 Hunter signals.
    if (request.method === "GET" && url.pathname === "/diagnostics/yesterday") {
      try {
        return json(await buildYesterdayPipelineDiagnostics(env));
      } catch (error) {
        return json({
          success: false,
          version: "V6.7.10.34",
    ...
    if (
      ...
      try {
        ...
        return json({
          success: true,
          version: "V6.7.10.34 TELEGRAM ALL FILTERED + FULL STATS",
          date: local.date,
          entry: Number(row?.total || 0),
          filter: "10-21_SCORE_GTE_64_BET_READY"
        });
      } catch (error) {
        return json({
          success: false,
    ...
    if (
      ...
      try {
        ...
        if (
          text === "/yesterdaycheck" ||
          text.startsWith("/yesterdaycheck@")
        ) {
          await sendTelegram(
            env,
            await buildYesterdayPipelineDiagnosticsMessage(env)
          );

          return json({
            success: true,
            action: "YESTERDAY_CHECK",
            mode: "READ_ONLY",
            population: "D1_10_21_SCORE_GTE_64"
          });
        }
        ...
        if (
          text === "/2025stats" ||
          text.startsWith("/2025stats@")
        ) {

          await sendTelegram(
            env,
            await build2025Stats(env)
          );

          return json({
            success: true,
            action: "2025STATS",
            population: "HUNTER_20_25_SCORE_GTE_60",
            live_strategy_changed: false,
            bet_ready_changed: false
          });
        }
...
async function createHunterEntry(
  ...
  // ==========================================================
  // V6.7.10.34 — SCORE 60–63 OBSERVATION ONLY
  ...
  // ==========================================================
  const liveRequiredScore = getRequiredHunterScore(minute);

  if (
    liveRequiredScore === null ||
    hunterScore < liveRequiredScore
  ) {
    console.log(
      "HUNTER OBSERVATION ONLY 60-63",
      id,
      matchName,
      minute,
      hunterScore
  ...
  // (10–21′ + Score >=64) is sent immediately, including
...
function isHunterCandidate(
  ...
  const requiredScore = getTrackingHunterScore(minute);
...
function getTrackingHunterScore(minute) {
  ...
  // getRequiredHunterScore() and remains 10–21′ + Score >=64.
...
function getRequiredHunterScore(minute) {
  ...
  // only 10–21′ with Hunter Score >=64.
  if (m >= 10 && m <= 21) return 64;
      ...
      .prepare(`
        SELECT
          ...
          CASE
            ...
            WHEN hunter_score BETWEEN 60 AND 64
              THEN '60–64'

            WHEN hunter_score BETWEEN 65 AND 69
              THEN '65–69'
...
🎯 10–21′: Score ≥64 | 22′+ НЕ СЕ ВЗИМАТ
🕐 Europe/Sofia`;
...
async function buildLiveHunterDiagnostics(env) {
  ...
  return {
    ...
    funnel: {
      ...
      score_64_plus: score64,
      already_tracking: alreadyTracking,
      new_eligible: newEligible
    },
    eligible,
    near_misses_score_below_64: nearMisses.slice(0, 20),
...
function previousSofiaDate(dateText) {
  const parts = String(dateText || "").split("-").map(Number);
  if (parts.length !== 3 || parts.some(v => !Number.isFinite(v))) return null;
  const d = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2] - 1, 12, 0, 0));
  return d.toISOString().slice(0, 10);
}

async function buildYesterdayPipelineDiagnostics(env) {
  if (!env.DB) throw new Error("DB binding missing");

  const today = getSofiaTime(new Date()).date;
  const date = previousSofiaDate(today);
  const bounds = getSofiaDayUtcBounds(date);
  if (!date || !bounds) throw new Error("INVALID_YESTERDAY_BOUNDS");

  const rowsResult = await env.DB.prepare(`
    SELECT id, match_id, match_name, league, entry_time, entry_minute,
           hunter_score, cloudbet_event_id, cloudbet_match, matcher_score,
           entry_odds, odds_available, telegram_message_id,
           status, result, goal_minute, goal_after_minutes,
  ...
  const classified = rows.map(row => {
    const eventId = row?.cloudbet_event_id !== null &&
      row?.cloudbet_event_id !== undefined &&
      String(row.cloudbet_event_id).trim() !== "";
    const odds = numberOrNull(row?.entry_odds);
    const oddsReady = eventId && odds !== null && odds > 1 &&
      Number(row?.odds_available || 0) === 1;
    const telegram = row?.telegram_message_id !== null &&
      row?.telegram_message_id !== undefined &&
      String(row.telegram_message_id).trim() !== "";

    let stage = "HUNTER_ONLY";
    if (eventId) stage = "MATCHED_NO_ODDS";
    if (oddsReady) stage = "BET_READY";

    return {
      match: row?.match_name || null,
      league: row?.league || null,
      entry_minute: Number(row?.entry_minute || 0),
      hunter_score: Number(row?.hunter_score || 0),
      stage,
      cloudbet_event_id: eventId ? String(row.cloudbet_event_id) : null,
      matcher_score: numberOrNull(row?.matcher_score),
      entry_odds: odds,
      odds_available: Number(row?.odds_available || 0) === 1,
      ...
      goal_minute: numberOrNull(row?.goal_minute),
      goal_after_minutes: numberOrNull(row?.goal_after_minutes),
      created_at: row?.created_at || null
    };
  });

  const count = stage => classified.filter(x => x.stage === stage).length;
  const matched = classified.filter(x => x.cloudbet_event_id !== null).length;
  const ready = classified.filter(x => x.stage === "BET_READY").length;
  const over150 = classified.filter(x => x.stage === "BET_READY" && x.entry_odds > 1.50).length;
  const atOrBelow150 = classified.filter(x => x.stage === "BET_READY" && x.entry_odds <= 1.50).length;
  ...
  return {
    ...
    date,
    timezone: TIME_ZONE,
    live_filter: "10-21_SCORE_GTE_64",
    note: "D1 can diagnose stored Hunter signals only; matches rejected before hunter_signals insertion are not reconstructable retrospectively.",
    summary: {
      hunter_entries: classified.length,
      unmatched: count("HUNTER_ONLY"),
      matched,
      matched_no_odds: count("MATCHED_NO_ODDS"),
      bet_ready: ready,
      bet_ready_odds_over_1_50: over150,
      bet_ready_odds_at_or_below_1_50: atOrBelow150,
      telegram_sent: classified.filter(x => x.telegram_sent).length,
      goals: classified.filter(x => x.result === "GOAL HIT").length,
      no_goals: classified.filter(x => x.result === "NO GOAL").length,
      open: classified.filter(x => x.result !== "GOAL HIT" && x.result !== "NO GOAL").length
    },
    signals: classified
  };
}

async function buildYesterdayPipelineDiagnosticsMessage(env) {
  const data = await buildYesterdayPipelineDiagnostics(env);
  const x = data.summary || {};
  let message = `🧪 YESTERDAY PIPELINE CHECK

📅 ${data.date}
🎯 LIVE FILTER: 10–21′ | Score >=64

📥 Hunter ENTRY: ${x.hunter_entries || 0}
❌ Unmatched: ${x.unmatched || 0}
🔗 Matched: ${x.matched || 0}
⏳ Matched / no odds: ${x.matched_no_odds || 0}
💰 BET READY: ${x.bet_ready || 0}

🎲 BET READY odds >1.50: ${x.bet_ready_odds_over_1_50 || 0}
🎲 BET READY odds <=1.50: ${x.bet_ready_odds_at_or_below_1_50 || 0}

📲 Telegram sent: ${x.telegram_sent || 0}
🟢 GOAL: ${x.goals || 0}
...
━━━━━━━━━━━━━━━━`;
  ...
  if (!data.signals?.length) {
    message += `\nНяма записани 10–21′ / Score >=64 Hunter сигнали за деня.`;
  } else {
    for (const row of data.signals) {
      const odds = row.entry_odds !== null && row.entry_odds !== undefined
        ? Number(row.entry_odds).toFixed(2) : "—";
      message += `\n\n${row.entry_minute}' | ${row.hunter_score}/100 | ${row.match}
${row.stage} | Event: ${row.cloudbet_event_id || "—"} | Odds: ${odds}
Telegram: ${row.telegram_sent ? "YES" : "NO"} | ${row.result || row.status || "—"}`;
    }
  }

  message += `\n\nℹ️ Това е D1 ретроспекция. Мачове, които не са стигнали до hunter_signals, не могат да се възстановят за вчера.`;
  return message;
}
...
// ============================================================
// V6.7.10.34 — /2025stats
// Research/reporting only.
// Population: stored Hunter signals at entry minute 20–25 and Score >=60.
// No odds requirement. No change to live Telegram, Matcher or BET READY.
// Test window starts 2026-09-25 Sofia so the new 10–42 / Score >=60
// capture can be evaluated without older strategy periods dominating.
// ============================================================

async function get2025StatsRows(env, bounds) {
  if (!bounds) return [];
  ...
  const result = await env.DB
    .prepare(`
      ...
      WHERE created_at >= ?
        AND created_at < ?
        AND entry_minute BETWEEN 20 AND 25
        AND hunter_score >= 60
      ORDER BY created_at ASC
    `)
    .bind(bounds.start, bounds.end)
...
function summarize2025Rows(rows) {
  ...
  const summarize = (subset) => {
    ...
    const goalTimes = subset
      .filter(x => x?.result === "GOAL HIT" && x?.goal_after_minutes !== null && x?.goal_after_minutes !== undefined)
      .map(x => Number(x.goal_after_minutes))
    ...
    return { total, goals, noGoals, open, resolved, rate, avgGoalAfter };
  };

  const total = summarize(source);

  const minutes = [];
  for (let minute = 20; minute <= 25; minute++) {
    minutes.push({
      label: `${minute}′`,
      minute,
      ...summarize(source.filter(x => Number(x?.entry_minute) === minute))
    });
  }

  // V6.7.10.34: exact Hunter Score values, not broad score buckets.
  // Include every integer score present in the 20–25′ sample, with a floor of 60–69
  // so the critical 64/65 boundary is always visible even when a value has 0 entries.
  const presentScores = source
    .map(x => Math.round(Number(x?.hunter_score)))
    .filter(score => Number.isFinite(score) && score >= 60 && score <= 100);

  const maxPresentScore = presentScores.length
    ? Math.max(...presentScores)
    : 69;

  const maxScore = Math.max(69, maxPresentScore);
  const scores = [];

  for (let score = 60; score <= maxScore; score++) {
    scores.push({
      label: String(score),
      score,
      ...summarize(
        source.filter(x => Math.round(Number(x?.hunter_score)) === score)
      )
    });
  }

  // Exact Score x Entry-minute matrix for 20/21/22/23/24/25.
  // Only scores with at least one entry are printed in the matrix to keep Telegram compact.
  const matrixScores = scores.filter(row => row.total > 0);
  const matrix = matrixScores.map(scoreRow => ({
    score: scoreRow.score,
    cells: minutes.map(minuteRow => ({
      minute: minuteRow.minute,
      ...summarize(
        source.filter(x =>
          Math.round(Number(x?.hunter_score)) === scoreRow.score &&
          Number(x?.entry_minute) === minuteRow.minute
        )
      )
    }))
  }));

  return { total, minutes, scores, matrix };
}


function format2025SummaryBlock(title, rows) {
  const stats = summarize2025Rows(rows);
  const s = stats.total;
...
NO GOAL: ${s.noGoals}
OPEN: ${s.open}
Success: ${s.rate.toFixed(1)}%
Avg time to GOAL: ${s.avgGoalAfter === null ? "—" : s.avgGoalAfter.toFixed(1) + " мин."}

⏱ ПО ТОЧНА ENTRY МИНУТА
`;

  for (const row of stats.minutes) {
    text += row.total
      ? `${row.label}: ${row.total} | ${row.goals}G ${row.noGoals}NG ${row.open}O | ${row.rate.toFixed(1)}%\n`
      : `${row.label}: 0 ENTRY\n`;
  }

  text += `\n🔥 ПО ТОЧЕН HUNTER SCORE\n`;

  for (const row of stats.scores) {
    text += row.total
      ? `${row.label}: ${row.total} | ${row.goals}G ${row.noGoals}NG ${row.open}O | ${row.rate.toFixed(1)}%\n`
      : `${row.label}: 0 ENTRY\n`;
  }
  ...
  if (!stats.matrix.length) {
    ...
    for (const row of stats.matrix) {
      const cells = row.cells.map(cell => {
        ...
        if (cell.open > 0 && cell.resolved === 0) return `${cell.total}O`;
        if (cell.noGoals === 0 && cell.open === 0) return `${cell.total}G`;
        if (cell.goals === 0 && cell.open === 0) return `${cell.total}NG`;
        return `${cell.goals}G/${cell.noGoals}NG${cell.open ? `/${cell.open}O` : ""}`;
      });
      text += `${row.score} | ${cells.join(" | ")}\n`;
    }
  }

  text += `\nG=GOAL | NG=NO GOAL | O=OPEN`;
  return text.trim();
}
